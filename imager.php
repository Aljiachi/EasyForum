...
$action = strip_tags(trim($_REQUEST['src']));

if($action == "captcha.png"){
	
	session_start();
	$rand = rand(00000000,99999999);
	$md5 = md5($rand);
	$text = substr($md5,0,7);
	$_SESSION['CapthaCode'] = $text;
	$image = imagecreate(80,30);
	$bg    = imagecolorallocate($image,255,255,255);
	$font  = imagecolorallocate($image,0,0,0);
	$gray =  imagecolorallocate($image, 229, 229, 229);
	$blue = imagecolorallocate($image,0,0,255); 
	$red  = imagecolorallocate($image,255,0,0); 
	imagestring($image,5,20,9,$text,$gray);
	imagestring($image,12,12,2,$text,$font);
...
define('MEMORY_TO_ALLOCATE',	'250M');
define('DEFAULT_QUALITY',		100);
define('CURRENT_DIR',			dirname(__FILE__));
define('CACHE_DIR_NAME',		'/includes/imagecache/');
...
if($maxWidth > $width){ $maxWidth = $width - 1;}
